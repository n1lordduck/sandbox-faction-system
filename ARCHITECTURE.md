...
Three sync strategies, chosen per message:

- **Chunked summary sync** (`SFS_SyncAll`) - sent once on `PlayerInitialSpawn` (and by the `faction_reload` concommand, via the shared `SFS.SyncAllToPlayer(ply)`). Every faction is sent, but only as a lightweight summary (`id, name, desc, icon, owner, public, friendlyFire, haloEnabled, memberCount` - built by `buildFactionSummary` in `sv_net.lua`), split into fixed-size chunks (`SYNC_CHUNK_SIZE`, currently 40 factions per net message) so a single message can never overflow GMod's ~64KB net cap regardless of how many factions exist. Each chunk carries a `syncGen` (generation id) plus its `index`/`total`; the client (`cl_data.lua`) merges chunks into `SFS.CL.Factions` as they arrive (firing `SFS_FactionsUpdated` each time, so the UI fills in progressively) and, once the last chunk of a generation lands, prunes any cached faction id that wasn't present in that generation - this is what keeps `faction_reload`-triggered deletions from lingering client-side. A faction object that only has summary fields (no `.members`) is detected via `SFS.CL.HasFullDetail(f)`.
- **Targeted full-detail push** (`SFS_SyncFaction`, one recipient) - `SFS.SyncFactionToPlayer(faction, ply)` in `sv_factions.lua` sends one faction's complete object (members, ranks, permissions, requests, allies, the lot) to a single player. `SFS.SyncAllToPlayer` calls this right after the summary chunks for the player's own faction and each of its direct allies, since halo/mate rendering (`cl_data.lua`'s `rebuildCache`) needs full member maps for those, not just summaries. Anything else that needs full detail on demand - currently just the Factions-browser detail view in `cl_panel.lua` - calls `SFS.CL.RequestFactionDetail(id)`, which nets `SFS_RequestFaction` to the server and gets the same targeted push back; this is the addon's one deliberate "lazy load" path; the browser shows a "Loading faction details..." placeholder until it lands and `SFS_FactionsUpdated` re-renders it.
- **Delta broadcast** (`SFS_SyncFaction`, everyone) - sent after *any* mutation to *one* faction, to *every* connected client, using the exact same message the targeted push uses (only the recipient differs - `net.Broadcast()` vs `net.Send(ply)`). Payload is either the updated faction JSON or the literal string `"__DELETED__"`. This is why lazy loading only had to be applied to the join-time full sync and not to live updates: every faction that mutates while you're connected ends up fully cached client-side anyway, broadcast to everyone exactly as before this changed. Clients merge this into `SFS.CL.Factions` and fire `SFS_FactionsUpdated`, which every open UI element listens to for live refresh.

There is no true delta/diffing on the full-detail push - "delta" here just means "resync one faction's full row" rather than the whole table. Wars use the older, unchunked pattern (`SFS_WarSync` / `SFS_WarsFullSync` / `SFS_WarRemoved`) - a single faction's or the whole war table's JSON in one message, uncompressed. This hasn't hit the same overflow bug in practice since there are normally far fewer concurrent wars than factions, but it's the same latent risk if war count ever grows large; nobody's touched it because nobody's reported it.

`SFS.CL.FactionMemberCount(f)` is the one function every UI list (Factions browser, Staff Panel, War tab side lists) uses to display a faction's member count - it reads `f.memberCount` when present (summary objects) and falls back to counting `f.members`/`f.subowners` (full objects, which don't carry a `memberCount` field). Anywhere that needs to display a count for an arbitrary faction should go through this rather than counting the raw maps directly, since those maps may not be populated yet.
